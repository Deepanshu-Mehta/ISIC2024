...
def test_single_member_group_ecdf_neutral() -> None:
    ...
    for feat in feature_cols:
        ...
        assert ecdf_val == pytest.approx(0.5), f"Single-lesion ecdf = {ecdf_val}, expected 0.5"
...
def test_no_nan_or_inf(base_config) -> None:
    ...
    new_cols = [
        ...
        if "_z_" in c or "_ecdf_" in c or c.startswith("ud_") or c == "is_single_lesion_patient"
    ]
...
def test_column_count(base_config) -> None:
    ...
    assert actual_new == expected_new, f"Expected {expected_new} new columns, got {actual_new}"
